        ...
        <script>
            $("#tbSearch").keyup(function () {
                ...
                $.ajax({
                    ...
                    data: { 'action': "search-categories", 'query': query },
                    dataType: "json",
                }).done(function (response) {
                    var tableHTML = "<select id='{{ $id }}' class='select_wide' size='10' name='{{ $element_name }}' >";
                    $.each(response, function (index, value) {
                        tableHTML += "<option value='" + index + "' class='match'>" + value + "</option>";
                    })
                    tableHTML += "</select>";
